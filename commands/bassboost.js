...
module.exports = {
    ...
    execute: async (client, args, user, channel, voiceChannel, message, prefix) => {
        ...
        queue.effects[`${filterToChange.toLowerCase()}`] = queue.effects[`${filterToChange.toLowerCase()}`] === 0 ? 7 : 0;
        ...
        return channel.send({
            content: translate(client, channel.guildId, "FILTER", queue.effects[`${filterToChange.toLowerCase()}`] !== 0, filterToChange) 
        }).catch(() => null);
    }
}